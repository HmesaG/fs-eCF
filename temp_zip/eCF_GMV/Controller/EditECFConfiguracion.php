...
class EditECFConfiguracion extends EditController
{
    ...
    {
        $this->addEditView('EditECFConfiguracion', \FacturaScripts\Plugins\eCF_GMV\Model\ECFConfiguracion::class, 'Configuración e-CF', 'fas fa-cog');
    ...
    protected function testCertificateAction(): bool
    {
        ...
        if ($certInfo) {
            ...
            $model->cert_vencimiento = $certInfo['vencimiento'];
            $model->save();
            $this->redirect(Tools::url('EditECFConfiguracion', ['code' => $model->id]));
            return false;
        }
